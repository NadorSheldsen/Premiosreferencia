   ...
   public class wwp_selectimportfile : GXWebComponent
   {
      ...
      protected void WB0I0( )
      {
         ...
         if ( ! wbLoad )
         {
            ...
            GxWebStd.gx_label_element( context, edtavFiltertoupload_Internalname, context.GetMessage( "File", ""), " AttributeLabel fieldLabel", 1, true, "");
            /* Div Control */
            ...
            ClassString = "Button";
            StyleString = "";
            GxWebStd.gx_button_ctrl( context, bttBtnucancel_Internalname, "", context.GetMessage( "Cancel", ""), bttBtnucancel_Jsonclick, 7, context.GetMessage( "Cancel", ""), "", StyleString, ClassString, 1, 1, "standard", "'"+sPrefix+"'"+",false,"+"'"+"e110i1_client"+"'", TempTags, "", 2, "HLP_WWPBaseObjects/WWP_SelectImportFile.htm");
            GxWebStd.gx_div_end( context, "start", "top", "div");
            GxWebStd.gx_div_end( context, "start", "top", "div");
            GxWebStd.gx_div_end( context, "start", "top", "div");
            GxWebStd.gx_div_end( context, "start", "top", "div");
         ...
         if ( ! String.IsNullOrEmpty(StringUtil.RTrim( AV7FilterToUpload)) )
         {
            if ( ( ( StringUtil.StrCmp(StringUtil.Upper( AV8FilterToUploadExt), "CSV") == 0 ) && ( StringUtil.StrCmp(AV9ImportType, "CSV") == 0 ) ) || ( ( StringUtil.StrCmp(StringUtil.Upper( AV8FilterToUploadExt), "XLSX") == 0 ) && ( StringUtil.StrCmp(AV9ImportType, "Excel") == 0 ) ) )
            {
               ...
               if ( new GeneXus.Programs.wwpbaseobjects.wwp_importdata(context).executeUdp(  AV14TransactionName,  AV9ImportType,  AV20BlobPath,  AV6ExtraParmsJson, out  AV5ErrorMsgs) )
               {
                  AV10LastErrorType = 2;
                  AV21GXV1 = 1;
                  while ( AV21GXV1 <= AV5ErrorMsgs.Count )
                  {
                     AV11Message = ((GeneXus.Utils.SdtMessages_Message)AV5ErrorMsgs.Item(AV21GXV1));
                  ...
                  GX_msglist.addItem(new WorkWithPlus.workwithplus_web.dvmessagegetbasicnotificationmsg(context).executeUdp(  context.GetMessage( "WWP_ImportFile_ImportSuccessTitle", ""),  AV12ResultMsg,  "success",  "",  "na",  ""));
               ...
               else
               {
                  ...
                  AssignProp(sPrefix, false, edtavFiltertoupload_Internalname, "URL", context.PathToRelativeUrl( AV7FilterToUpload), true);
                  AV22GXV2 = 1;
                  while ( AV22GXV2 <= AV5ErrorMsgs.Count )
                  {
                     AV11Message = ((GeneXus.Utils.SdtMessages_Message)AV5ErrorMsgs.Item(AV22GXV2));
                     if ( ! String.IsNullOrEmpty(StringUtil.RTrim( AV12ResultMsg)) )
                     {
                        AV12ResultMsg += StringUtil.NewLine( );
                  ...
                  GX_msglist.addItem(new WorkWithPlus.workwithplus_web.dvmessagegetbasicnotificationmsg(context).executeUdp(  context.GetMessage( "WWP_ImportFile_ErrorImportingTitle", ""),  AV12ResultMsg,  "error",  "",  "false",  ""));
               }
            }
            else
            {
               AV7FilterToUpload = "";
               AssignProp(sPrefix, false, edtavFiltertoupload_Internalname, "URL", context.PathToRelativeUrl( AV7FilterToUpload), true);
               AV12ResultMsg = StringUtil.Format( context.GetMessage( "WWP_ImportFile_ExpectedFileType", ""), ((StringUtil.StrCmp(AV9ImportType, "CSV")==0) ? "csv" : "xlsx"), "", "", "", "", "", "", "", "");
               GX_msglist.addItem(new WorkWithPlus.workwithplus_web.dvmessagegetbasicnotificationmsg(context).executeUdp(  context.GetMessage( "WWP_ImportFile_ErrorImportingTitle", ""),  AV12ResultMsg,  "error",  "",  "na",  ""));
            }
         }
         else
         {
      ...
      protected void define_styles( )
      {
         ...
         {
            context.AddJavascriptSource(StringUtil.RTrim( ((string)Form.Jscriptsrc.Item(idxLst))), "?2025102815461636", true, true);
            idxLst = (int)(idxLst+1);
         }
         if ( ! outputEnabled )
         {
      ...
      {
         context.AddJavascriptSource("wwpbaseobjects/wwp_selectimportfile.js", "?2025102815461636", false, true);
