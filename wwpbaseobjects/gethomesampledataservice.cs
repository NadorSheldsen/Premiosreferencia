...
namespace GeneXus.Programs.wwpbaseobjects {
   public class gethomesampledataservice : GXWebProcedure
   {
      public override void webExecute( )
      {
         ...
         initialize();
         Gxm2rootcol = new GXBaseCollection<WorkWithPlus.workwithplus_web.SdtHomeSampleData_HomeSampleDataItem>( context, "HomeSampleDataItem", "Premios") ;
         ...
         if ( StringUtil.StrCmp(StringUtil.Lower( GXSoapHTTPRequest.Method), "get") == 0 )
         {
            if ( StringUtil.StrCmp(StringUtil.Lower( GXSoapHTTPRequest.QueryString), "wsdl") == 0 )
            {
               ...
               GXSoapXMLWriter.WriteAttribute("soapAction", "Premiosaction/"+"wwpbaseobjects.AGETHOMESAMPLEDATASERVICE.Execute");
         ...
         if ( ( StringUtil.StringSearch( currMethod+"&", "Execute&", 1) > 0 ) || ( currSoapErr != 0 ) )
         {
            ...
            if ( currSoapErr == 0 )
            {
               if ( Gxm2rootcol != null )
               {
                  Gxm2rootcol.writexmlcollection(GXSoapXMLWriter, "ReturnValue", "Premios", "HomeSampleDataItem", "Premios");
               }
            }
            else
            {
      ...
      {
         this.Gxm2rootcol = new GXBaseCollection<WorkWithPlus.workwithplus_web.SdtHomeSampleData_HomeSampleDataItem>( context, "HomeSampleDataItem", "Premios") ;
         initialize();
         ExecuteImpl();
